  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-4">
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => logout()}
              className="px-4 py-2.5 text-[14px] font-medium text-slate-600 hover:text-rose-600 transition-colors"
            >
              Logout
            </button>
          </div>
      ...
      <div className="relative md:hidden" ref={menuRef}>
        ...
        {isOpen && (
          <div className="absolute right-0 top-full mt-2.5 bg-white rounded-[24px] shadow-2xl border border-slate-100/90 p-2 flex flex-col gap-1 w-[160px] text-center z-50 animate-in fade-in zoom-in-95 duration-150">
            {navLinks.map((link) => {
            ...
            {user ? (
              <>
                <Link
                  href="/dashboard"
                  onClick={() => setIsOpen(false)}
                  className="w-full py-2 px-3 rounded-full text-[13.5px] font-medium bg-[#059669] text-white"
                >
                  Dashboard
                </Link>
                <button
                  ...
                  className="w-full py-2 px-3 rounded-full text-[13.5px] font-medium text-rose-600 hover:bg-rose-50"
                >
                  Logout
                </button>
              </>
              ...
              <>
                <Link
                  href="/login"
                  onClick={() => setIsOpen(false)}
                  className="w-full py-2 px-3 rounded-full text-[13.5px] font-medium text-[#059669] hover:bg-emerald-50"
                >
                  Login
                </Link>
                <Link
